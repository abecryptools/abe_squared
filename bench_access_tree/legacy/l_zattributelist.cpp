...
#define __L_OpenABEATTRIBUTELIST_CPP__
...
L_OpenABEAttributeList::L_OpenABEAttributeList() : L_OpenABEFunctionInput() {
...
L_OpenABEAttributeList::L_OpenABEAttributeList(uint32_t numArgs, std::vector<string> args)
    : L_OpenABEFunctionInput() {
  string c;
...
L_OpenABEAttributeList::L_OpenABEAttributeList(const L_OpenABEAttributeList &copy) {
...
L_OpenABEAttributeList::~L_OpenABEAttributeList() { this->m_Attributes.clear(); }
...
void L_OpenABEAttributeList::getStringList(std::vector<string> &attrStrings) {
...
void L_OpenABEAttributeList::syncOrigAttributes(const string &prefix,
                                          L_OpenABEAttributeList &attrList) {
...
bool L_OpenABEAttributeList::matchAttribute(const string &attribute) {
  return (std::find(m_Attributes.begin(), m_Attributes.end(), attribute) !=
          m_Attributes.end());
}

bool L_OpenABEAttributeList::addAttribute(string attribute) {
  string c, str;
  ...
  if (attribute.find(EQUALS) == string::npos) {
    ...
    std::unique_ptr<L_OpenABEAttributeList> attr_list = L_createAttributeList(ATTR_SEP + attribute);
...
void L_OpenABEAttributeList::setAttributes(vector<string> &attr_list,
...
ostream &operator<<(ostream &os, const L_OpenABEAttributeList &attributeList) {
  int i = 0;
  L_OpenABEAttributeList attributeList2 = attributeList;
...
std::string L_OpenABEAttributeList::l_toString() const {
  string s;
...
std::string L_OpenABEAttributeList::l_toCompactString() const {
  string s;
  s.push_back(ATTR_SEP);
...
void L_OpenABEAttributeList::serialize(L_OpenABEByteString &result) const {
  result = this->l_toCompactString();
}

void L_OpenABEAttributeList::deserialize(const L_OpenABEByteString &input) {}

bool L_OpenABEAttributeList::isEqual(L_ZObject *z) const {
  L_OpenABEAttributeList *z1 = dynamic_cast<L_OpenABEAttributeList *>(z);
  if (z1 != NULL) {
