...
void Driver::set_policy(L_OpenABETreeNode *subtree) {
  if (this->final_policy == nullptr) {
    this->final_policy = std::unique_ptr<L_OpenABEPolicy>(new L_OpenABEPolicy);
    this->final_policy->l_setRootNode(subtree);
  } else {
    this->final_policy->l_setRootNode(subtree);
  }
  if (this->debug)
    std::cout << "Final policy set!" << std::endl;
  this->final_policy->l_setDuplicateInfo(this->attr_count, this->attr_dup);
  if (this->attr_prefix.size() > 0) {
    this->final_policy->l_setPrefixSet(this->attr_prefix);
  }
  this->final_policy->l_setCompactString(this->original_input);
}
...
void Driver::set_attrlist(std::vector<std::string> *attr_list) {
  ...
  this->final_attrlist.reset(new L_OpenABEAttributeList);
...
vector<string> *Driver::attr_num(const std::string &c, L_OpenABEUInteger *number) {
...
L_OpenABETreeNode *Driver::leaf_node(const std::string &c) {
  int index = 0;
  if (this->debug) {
  ...
  return new L_OpenABETreeNode(attribute, prefix, index);
}

L_OpenABETreeNode *Driver::kof2_tree(int k, L_OpenABETreeNode *l, L_OpenABETreeNode *r) {
  L_OpenABETreeNode *rootNode = new L_OpenABETreeNode();
  zGateType node_type;
  ...
  rootNode->l_setNodeType(node_type);
  rootNode->l_addSubnode(l);
  rootNode->l_addSubnode(r);
  rootNode->l_setThresholdValue(k);
  if (this->debug) {
...
L_OpenABETreeNode *Driver::kofn_tree(uint32_t threshold_k,
                               std::vector<L_OpenABETreeNode *> &attributeList) {
  L_OpenABETreeNode *rootNode = new L_OpenABETreeNode();
  zGateType node_type;
  size_t k;
  ...
  for (k = 0; k < attributeList.size(); k++) {
    if (this->debug)
      std::cout << "Subnode " << k << ": " << attributeList[k]->l_toString()
                << std::endl;
    rootNode->l_addSubnode(attributeList[k]);
  }
  rootNode->l_setThresholdValue(threshold_k);
  if (this->debug)
...
L_OpenABEUInteger *create_expint(uint32_t value, uint16_t bits) {
  return new L_OpenABEUInteger(value, bits);
}

L_OpenABEUInteger *create_flexint(uint32_t value) {
  return new L_OpenABEUInteger(value, MAX_INT_BITS);
}
...
L_OpenABETreeNode *Driver::bit_marker_list(bool flex, bool gt, std::string attr,
                                     int bits, uint32_t value) {
  L_OpenABETreeNode *p = NULL;
  int i;
...
L_OpenABETreeNode *Driver::flexint_leader(bool gt, std::string attr, uint32_t value) {
  // printf("called flexint_leader: gt=%d, attr=%s, value=%d\n", gt, attr,
  // value);
  int k;
  std::vector<L_OpenABETreeNode *> attributes;
  uint32_t i = 0;
...
L_OpenABETreeNode *Driver::cmp_policy(L_OpenABEUInteger *number, bool gt,
                                std::string attr) {
  L_OpenABETreeNode *p = NULL;
...
L_OpenABETreeNode *Driver::eq_policy(const std::string &c, L_OpenABEUInteger *number) {
  L_OpenABETreeNode *p = NULL;
  int bits = number->getBits();
...
L_OpenABETreeNode *Driver::lt_policy(const std::string &attr, L_OpenABEUInteger *number) {
  return this->cmp_policy(number, false, attr);
}

L_OpenABETreeNode *Driver::gt_policy(const std::string &attr, L_OpenABEUInteger *number) {
  return this->cmp_policy(number, true, attr);
}

L_OpenABETreeNode *Driver::le_policy(const std::string &attr, L_OpenABEUInteger *number) {
  *number += 1;
  return this->cmp_policy(number, false, attr);
}

L_OpenABETreeNode *Driver::ge_policy(const std::string &attr, L_OpenABEUInteger *number) {
  *number -= 1;
  return this->cmp_policy(number, true, attr);
}

L_OpenABETreeNode *Driver::range_policy(const std::string &c, L_OpenABEUInteger *min_num,
                                  L_OpenABEUInteger *max_num) {
  ...
  L_OpenABETreeNode *rootNode = new L_OpenABETreeNode();
  L_OpenABETreeNode *l = this->gt_policy(c, min_num);
  L_OpenABETreeNode *r = this->lt_policy(c, max_num);
  rootNode->l_setNodeType(GATE_TYPE_AND);
  rootNode->l_addSubnode(l);
  rootNode->l_addSubnode(r);
  rootNode->l_setThresholdValue(2);
  return rootNode;
}

L_OpenABETreeNode *Driver::range_incl_policy(const std::string &c,
                                       L_OpenABEUInteger *min_num,
                                       L_OpenABEUInteger *max_num) {
  if (min_num->getVal() > max_num->getVal()) {
    //throw OpenABE_ERROR_INVALID_RANGE_NUMBERS;
  } else if (min_num->getBits() != max_num->getBits()) {
  ...
  // translate to (LEAF >= min_num AND LEAF <= max_num)
  L_OpenABETreeNode *rootNode = new L_OpenABETreeNode();
  L_OpenABETreeNode *l = this->ge_policy(c, min_num);
  L_OpenABETreeNode *r = this->le_policy(c, max_num);
  rootNode->l_setNodeType(GATE_TYPE_AND);
  rootNode->l_addSubnode(l);
  rootNode->l_addSubnode(r);
  rootNode->l_setThresholdValue(2);
...
L_OpenABEUInteger *get_month(const string &month) {
  int m = -1;
  if (month == "January" || month == "Jan")
    ...
    m = 0;
  return new L_OpenABEUInteger(m, MAX_INT_BITS);
}

uint32_t validate_date(const std::string &prefix, L_OpenABEUInteger *m,
                       L_OpenABEUInteger *d, L_OpenABEUInteger *y) {
...
void validate_range_date(const std::string &prefix, L_OpenABEUInteger *m,
                         L_OpenABEUInteger *min_d, L_OpenABEUInteger *max_d,
                         L_OpenABEUInteger *y) {
  if (prefix == MONTH_KEYWORD || prefix == DAY_KEYWORD ||
      prefix == YEAR_KEYWORD) {
    //throw OpenABE_ERROR_INVALID_PREFIX_SPECIFIED;
...
L_OpenABETreeNode *Driver::set_date_in_policy(const std::string &prefix,
                                        L_OpenABEUInteger *m, L_OpenABEUInteger *d,
                                        L_OpenABEUInteger *y) {
  // date = {Month} {Day}, {Year}
  ...
  L_OpenABEUInteger ui(s, 32);
  L_OpenABETreeNode *rootNode = this->eq_policy(attr, &ui);
  return rootNode;
}

L_OpenABETreeNode *Driver::gt_date_in_policy(const std::string &prefix,
                                       L_OpenABEUInteger *m, L_OpenABEUInteger *d,
                                       L_OpenABEUInteger *y) {
  // date > {Month} {Day}, {Year}
  uint32_t s = validate_date(prefix, m, d, y);
  ...
  L_OpenABEUInteger ui(s, 32);
  L_OpenABETreeNode *rootNode = this->gt_policy(attr, &ui);
  return rootNode;
}

L_OpenABETreeNode *Driver::ge_date_in_policy(const std::string &prefix,
                                       L_OpenABEUInteger *m, L_OpenABEUInteger *d,
                                       L_OpenABEUInteger *y) {
  // date >= {Month} {Day}, {Year}
  uint32_t s = validate_date(prefix, m, d, y);
  ...
  L_OpenABEUInteger ui(s, 32);
  L_OpenABETreeNode *rootNode = this->ge_policy(attr, &ui);
  return rootNode;
}

L_OpenABETreeNode *Driver::lt_date_in_policy(const std::string &prefix,
                                       L_OpenABEUInteger *m, L_OpenABEUInteger *d,
                                       L_OpenABEUInteger *y) {
  // date < {Month} {Day}, {Year}
  uint32_t s = validate_date(prefix, m, d, y);
  ...
  L_OpenABEUInteger ui(s, 32);
  L_OpenABETreeNode *rootNode = this->lt_policy(attr, &ui);
  return rootNode;
}

L_OpenABETreeNode *Driver::le_date_in_policy(const std::string &prefix,
                                       L_OpenABEUInteger *m, L_OpenABEUInteger *d,
                                       L_OpenABEUInteger *y) {
  // date < {Month} {Day}, {Year}
  uint32_t s = validate_date(prefix, m, d, y);
  ...
  L_OpenABEUInteger ui(s, 32);
  L_OpenABETreeNode *rootNode = this->le_policy(attr, &ui);
  return rootNode;
}

L_OpenABETreeNode *Driver::range_date_in_policy(const std::string &prefix,
                                          L_OpenABEUInteger *m, L_OpenABEUInteger *min_d,
                                          L_OpenABEUInteger *max_d, L_OpenABEUInteger *y) {
  ...
  L_OpenABEUInteger ui_min((uint32_t)s1_days, 32), ui_max((uint32_t)s2_days, 32);
  L_OpenABETreeNode *l = this->ge_policy(attr, &ui_min);
  L_OpenABETreeNode *r = this->le_policy(attr, &ui_max);
  L_OpenABETreeNode *rootNode = new L_OpenABETreeNode();
  rootNode->l_setNodeType(GATE_TYPE_AND);
  rootNode->l_addSubnode(l);
  rootNode->l_addSubnode(r);
...
std::ostream &Driver::print(std::ostream &stream) {
  stream << this->final_policy->l_getRootNode()->l_toString() << "\n";
  return (stream);
}
