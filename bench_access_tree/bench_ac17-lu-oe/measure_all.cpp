...
int main(int argc, char **argv) {
    ...
    bn_t r; bn_null(r); bn_new(r);   bn_t r_mul_b; bn_null(r_mul_b); bn_new(r_mul_b);
  ...
  // XXX: only for generating the right order in the loop
    ...
    g2_t C_2; g1_null(C_2); g1_new(C_2);


    struct ciphertext_lu_oe CT_A;
    init_ciphertext_lu_oe(N_ATTR, &CT_A);
    gt_new(CT_A.C);
