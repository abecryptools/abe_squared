...
unsigned char * uint32_to_u_char_array(const uint32_t n)                      {                                             
       ...
       a[0] = (n >> 24) & 0xff;      
       a[1] = (n >> 16) & 0xff;                                                      a[2] = (n >>  8) & 0xff;                                                                                                                     a[3] = n         & 0xff;                                                                                                                                                                                                              
       return a;                                                                }     
...
int main(int argc, char **argv) {
  ...
  cout << "\tencInput: " << encInput << endl;

      core_init();                                                                                                                                   
     bn_t order;                                                                                                                                       pc_param_set_any();                                                                                                                            
     pc_param_print();                                                                                                                                  
     pc_get_ord(order);
  ...
  struct public_key_wat11_iv_ok mpk;
  ...
  struct secret_key_wat11_iv_ok sk;                                     
  init_secret_key_wat11_iv_ok(N_ATTR, &sk);   
  ...
  struct ciphertext_wat11_iv_ok CT_A;                                
  init_ciphertext_wat11_iv_ok(N_ATTR, &CT_A);
  ...
  */ 
  ...
  g1_t pack_g1[N_ATTR];                                           
  bn_t pack_bn[N_ATTR];                                                
  g1_t pairing_g1[2 + N_ATTR];
  g2_t pairing_g2[2 + N_ATTR];
  ...
  for(int j=0; j<NTESTS; j++) {
      ...
      for (auto it = lsssRows.begin(); it != lsssRows.end(); ++it) {     
          ...
          get_k_attr_wat11_iv_ok(attr_int_1, sk, k_attr); 
          ...
          g2_neg(pairing_g2[i], pairing_g2[i]);
          i++;
      }      
