...
int main(int argc, char **argv) {
    ...
    for(int j=0; j<NTESTS; j++) {                                                                                                                            
        ...
        for (auto it = attrStrings->begin(); it != attrStrings->end(); ++it    ) {      
            ...
            bn_t_mul(r_mul_b_prima, r, msk.b_prima, order);                        bn_t_mul(tmp_t, msk.b_1, attr_int_z.m_ZP, order);
    ...
    for(int j=0; j<NTESTS; j++) {
        ...
        for (auto it = lsssRows.begin(); it != lsssRows.end(); ++it) {     
            ...
            g1_mul_fix(CT_A.C_1[i].c_attr, t_pre_g_b, it->second.element().m_ZP);
    ...
    for(int j=0; j<NTESTS; j++)  {
        ...
        for (auto it = lsssRows.begin(); it != lsssRows.end(); ++it) {     
            ...
            get_k_attr_rw13_oe(attr_int, sk, k_attr_1, k_attr_2);
            ...
            g1_mul(CT_A.C_3[i].c_attr, CT_A.C_3[i].c_attr, it->second.element().m_ZP);

            g1_copy(p_1[i], CT_A.C_2[i].c_attr);
            g2_copy(p_2[i], k_attr_2);

            g1_neg(pairing_g1_d[i], CT_A.C_3[i].c_attr);
            g2_copy(pairing_g2_d[i], k_attr_1);
        ...
        // comment for checking correctness
    } print_results("Results encryption():           ", t, NTESTS);
