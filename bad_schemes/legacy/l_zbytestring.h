...
#ifndef __L_ZBYTESTRING_H__
#define __L_ZBYTESTRING_H__
...
class L_OpenABEByteString : public L_ZObject, public std::vector<uint8_t> {
	
public:
  L_OpenABEByteString& operator+=(const L_OpenABEByteString &concat) {
    this->insert(this->end(), concat.begin(), concat.end());
    return *this;
  }

  L_OpenABEByteString& operator+=(const std::string concat) {
    this->insert(this->end(), concat.begin(), concat.end());
    return *this;
  }

  L_OpenABEByteString& appendArray(uint8_t input[], uint32_t size) {
    this->insert(this->end(), &input[0], &input[size]);
    return *this;
  }

  L_OpenABEByteString operator+(L_OpenABEByteString &concat) {
    L_OpenABEByteString result;
    result.insert(result.end(), this->begin(), this->end());
    result.insert(result.end(), concat.begin(), concat.end());
    return result;
  }

  L_OpenABEByteString operator+(std::string concat) {
    L_OpenABEByteString result;
  ...
  L_OpenABEByteString&    operator=(const std::string &rhs) {
    this->clear();
    this->insert(this->end(), rhs.begin(), rhs.end());
    return *this;
  }

  L_OpenABEByteString& operator^=(L_OpenABEByteString &rhs) {
  ...
  L_OpenABEByteString getSubset(size_t start_pos, size_t num_bytes) {
    L_OpenABEByteString result;
  ...
  L_OpenABEByteString* clone() const {
    return new L_OpenABEByteString(*this);
  }
  ...
  friend bool operator==(const L_OpenABEByteString& lhs, const L_OpenABEByteString& rhs) {
  ...
  friend std::ostream& operator<<(std::ostream& s, const L_OpenABEByteString& z) {
    L_OpenABEByteString z2(z);
  ...
  bool isEqual(L_ZObject* z) const {
    L_OpenABEByteString *z1 = dynamic_cast<L_OpenABEByteString*>(z);
    if(z1 != NULL) {
      return *z1 == *this;
    }
    return false;
  ...
  void pack(L_OpenABEByteString &buf) {
  ...
  void smartPack(L_OpenABEByteString& buf) {
  ...
  L_OpenABEByteString smartUnpack(size_t *index) {
    ...
    L_PackType pack_type = (L_PackType) this->at(*index);
    *index += 1;
    if(pack_type == L_PACK_32) {
      return this->unpack(index);
    } else if(pack_type == L_PACK_16) {
      return this->unpack16bits(index);
    } else if(pack_type == L_PACK_8) {
      return this->unpack8bits(index);
    } else {
      std::cerr << "Pack type: " << pack_type << std::endl;
      //THROW_ERROR(OpenABE_ERROR_INVALID_PACK_TYPE);
    }
    L_OpenABEByteString buf;
    return buf;
  }

  L_OpenABEByteString unpack8bits(size_t *index) {
    size_t index2 = *index;
    L_OpenABEByteString buf;
    size_t buf_len = this->size();
  ...
  L_OpenABEByteString unpack16bits(size_t *index) {
    size_t index2 = *index;
    L_OpenABEByteString buf;
    size_t buf_len = this->size();
  ...
  L_OpenABEByteString unpack(size_t *index) {
    size_t index2 = *index;
    L_OpenABEByteString buf;
    size_t buf_len = this->size();
  ...
  void unpack(size_t *index, L_OpenABEByteString & buf) {
    size_t index2 = *index;
    size_t buf_len = this->size();
    if (index2 >= buf_len || (index2 + 4) > buf_len) {
      //THROW_ERROR(OpenABE_ERROR_INDEX_OUT_OF_BOUNDS);
